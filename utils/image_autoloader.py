...
def _open_file_picker(initial_dir: str = ""):
    if sys.platform.startswith("win"):
        selected = _open_file_picker_windows(initial_dir)
        if selected is not None:
            return selected
...
def _open_file_picker_windows(initial_dir: str = ""):
    script = r'''
Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
Add-Type @"
...
public static class FileForegroundHelper {
    [DllImport("user32.dll")] public static extern bool SetForegroundWindow(IntPtr hWnd);
    [DllImport("user32.dll")] public static extern IntPtr GetForegroundWindow();
    [DllImport("user32.dll")] public static extern uint GetWindowThreadProcessId(IntPtr hWnd, out uint processId);
    [DllImport("kernel32.dll")] public static extern uint GetCurrentThreadId();
...
$ownerForm.Show()
$ownerForm.Activate()
[FileForegroundHelper]::ForceForeground($ownerForm.Handle)

$dialog = New-Object System.Windows.Forms.OpenFileDialog
$dialog.Title = "Select image file"
$dialog.Filter = "Image files (*.png;*.jpg;*.jpeg;*.webp;*.bmp;*.tiff;*.tif;*.gif)|*.png;*.jpg;*.jpeg;*.webp;*.bmp;*.tiff;*.tif;*.gif|All files (*.*)|*.*"
$initialDir = $env:GHTOOLS_INITIAL_DIR
if ($initialDir -and (Test-Path $initialDir)) {
...
class ImageAutoloader(PreviewImage):
    ...
    @classmethod
    def IS_CHANGED(cls, folder_path="", sort_by="time", sort_order="descending", image_path="", **kwargs):
        ip = image_path.strip().strip('"')
        if ip and os.path.isfile(ip):
            return f"file_{os.path.getmtime(ip)}_{os.path.basename(ip)}"
    ...
    def load_latest(self, folder_path="", sort_by="time", sort_order="descending",
                    image_path="", prompt=None, extra_pnginfo=None):
        ip = image_path.strip().strip('"')
        if ip and os.path.isfile(ip):
            target = ip
        else:
...
@PromptServer.instance.routes.post("/ghtools/image_autoloader_pick_image")
async def handle_image_autoloader_pick_image(request):
    try:
        data = await request.json()
    except Exception:
        data = {}
    ...
    try:
        selected = await asyncio.to_thread(_open_file_picker, initial_dir)
    except Exception as exc:
        return web.json_response({"code": -1, "error": str(exc)})

    if not selected:
        return web.json_response({"code": 0, "file_path": ""})

    return web.json_response({"code": 1, "file_path": os.path.normpath(selected)})


@PromptServer.instance.routes.post("/ghtools/image_autoloader_load_image")
async def handle_image_autoloader_load_image(request):
    try:
        data = await request.json()
    except Exception:
        data = {}

    file_path = str(data.get("file_path", "")).strip().strip('"')
    if not file_path or not os.path.isfile(file_path):
        return web.json_response({"code": 0, "error": "Invalid file path"})

    ext = os.path.splitext(file_path)[1].lower()
    if ext not in IMAGE_EXTENSIONS:
        return web.json_response({"code": 0, "error": "Not an image file"})

    try:
        preview = await asyncio.to_thread(_create_preview_image_info, file_path)
    except Exception as exc:
        return web.json_response({"code": -1, "error": str(exc)})

    return web.json_response({"code": 1, "image": preview, "source_path": file_path})
